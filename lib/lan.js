 ...
 *
 * ## An accepted join is not a delivered datagram
 *
 * Measured host to host across a direct ethernet segment: a Windows sender on
 * `192.168.137.1` sent eight frames to the group, this Mac's listener joined
 * reporting `(default)=ok` and `192.168.137.2=ok`, `tcpdump -n -i en8` saw all
 * eight arrive with `0 packets dropped by kernel`, and the application received
 * **none**. The frames reach the NIC and the socket does not hand them up.
 *
 * **The sharpest true statement is narrower than "`addMembership` returning `ok`
 * is not a join", and the narrowing is measured rather than argued.** `ok` means
 * `setsockopt(IP_ADD_MEMBERSHIP)` returned 0, and that is not vacuous: on this
 * host `addMembership(group, '203.0.113.7')` throws `EADDRNOTAVAIL` because no
 * interface holds that address, and `addMembership(group, '10.65.28.161')` throws
 * `EADDRINUSE` because the default join already took `en0` — which is itself the
 * proof that the kernel resolved the default join through `224.0.0.0/4` to the
 * Wi-Fi. So an `ok` *does* say a membership now exists for that interface. What it
 * does not say is that a datagram arriving on that interface will be delivered to
 * this socket, and on the measured segment it was not.
 *
 * That residue is left unexplained rather than attributed, and the correction is
 * worth keeping: routes do not gate multicast *ingress* on this platform —
 * per-interface membership does — so `224.0.0.0/4` pointing at `en0` is a
 * send-side fact and cannot be the receive-side mechanism. The earlier note read
 * it as one.
 *
 * ## What one host can measure about itself is the interface it announces on
 *
 * And it is free. The kernel loops a copy of every multicast datagram back to the
 * sending host, so a beacon already receives its own frame and already throws it
 * away by nonce. Reading `rinfo.address` off that echo turns the fact that took
 * two machines and `tcpdump` to establish into a field — `announcing` — that any
 * single device can read about itself. Measured here: memberships accepted on
 * `en0` *and* `en8`, `announcing` = `10.65.28.161`, which is `en0`. This device
 * joined a segment it does not announce on, and now says so.
 *
 * The limits, in the same breath, because this is a diagnostic and not a health
 * check. Hearing your own echo says nothing about whether a neighbour hears you or
 * whether you hear a neighbour — it is one host's kernel talking to itself.
 * `announcing` stays `null` where multicast loopback is off, which is a host this
 * cannot measure and not a host that is broken. And it is taken from an
 * unauthenticated frame that merely carries our nonce, so a neighbour replaying
 * one makes this device *report* the wrong interface; it changes no behaviour,
 * because nothing branches on this field, and the first echo wins, which arrives
 * microseconds after the first announce and before anybody could have observed a
 * nonce to replay.
 *
 * `memberships` is the other half and is named for what it holds: the joins that
 * were **accepted**, never the joins that were verified. There is no state here
 * that reads as "the segment works", which is the whole point — `joined = 1` was a
 * local counter used for one throw, and the operator-facing rendering built on top
 * of it (`artifact-operator/lib/serve.js`, `segment: on`) is the surface where an
 * accepted join has been reading as health all along.
 *
 * ## Three things this deliberately does not do
 *
 * **Declaring the beacon up only once it hears its own echo was considered and is
 * rejected on the measurement above.** The echo arrives over loopback, against the
 * *outgoing* interface's membership, so on the very Mac whose segment is broken
 * today that probe returns a clean yes in under a second: it would have been the
 * fifth instrument of the day reporting success without measuring the thing it
 * named. It fails the other way too, on a host with `IP_MULTICAST_LOOP` off —
 * which is not settable from JS either — where it turns a working device red. A
 * round trip that cannot reach the wire is not a round trip.
 *
 * **Refusing to open was rejected on `OPERATIONS.md`'s standing posture**: warn
 * rather than refuse for what the platform cannot enforce. A beacon that will not
 * start takes LAN discovery away from every multi-homed host in order to report a
 * fault present on some of them, and the platform's own procedures run over it.
 *
 * **Steering the interface was not rejected — it is not reachable.** The ceiling
 * registered at the join in `Beacon.open` is where that is written down, and it
 * is deliberately not named by its marker here: the reverse half of
 * `--check-ledger` sweeps this directory for that word and would read a prose
 * mention as a second unregistered ceiling, which is one exemption in somebody
 * else's script rather than one sentence phrased around it.
 */
...
class Beacon {
   ...
   * @param {string[]} parts.memberships
   * @param {(peer: { host: string, port: number }) => void} [parts.onpeer]
   */
  constructor ({ socket, port, nonce, group, socketPort, memberships, onpeer }) {
    this.socket = socket
    this.port = port
    this.nonce = nonce
    this.group = group
    ...
    /**
     * The joins the kernel **accepted**, never the joins anything verified — the
     * header says what the difference is and what was measured. An entry is the
     * interface address that was asked for; `''` is the join the kernel resolved
     * itself through the multicast route, and `announcing` below is the only
     * thing on this object that can name which interface that turned out to be.
     *
     * @type {string[]}
     */
    ...
    /**
     * The local address this device's own beacons are leaving from, read off the
     * kernel's loopback copy of one of them, or `null` where no echo has arrived.
     *
     * A single host cannot measure whether its beacons reach anybody. It can
     * measure which interface they left on, and on a multi-homed host that is the
     * fault worth naming: a device holding an accepted membership on an interface
     * it does not announce on is a device invisible on that segment. `null` means
     * unmeasured — multicast loopback off, or nothing sent yet — and not broken.
     *
     * @type {string | null}
     */
  ...
  static async open (opts) {
    ...
    // a multi-homed host that guesses wrong receives nothing at all.
    //
    // What is kept is the list of joins the kernel *accepted*, and the name says
    // that rather than "joined": measured on this host, an `ok` means a membership
    // exists for that interface — a bogus address throws `EADDRNOTAVAIL` and a
    // duplicate throws `EADDRINUSE` — and does **not** mean a datagram arriving
    // there will be delivered. The header carries the two-host measurement where
    // it was not.
    //
    // ponytail: multicast cannot be steered from JS in either direction. `udx-native/lib/socket.js` exposes `addMembership`, `dropMembership` and `setTTL` and nothing else — no `IP_MULTICAST_IF`, no `IP_MULTICAST_LOOP`, no `IP_MULTICAST_TTL` — so a multi-homed host announces on whatever `224.0.0.0/4` routes to and joins every interface hoping one of them is the right one. Measured on this host: joins accepted on `en0` and `en8`, frames leaving on `en0`. Bind-to-address is measured and is not a workaround, it changed nothing. The upgrade path is concrete and upstream — libuv has `uv_udp_set_multicast_interface`, and `binding.cc`'s `udx_napi_socket_set_membership` is already the same shape of passthrough, so it is one napi function and one method on `Socket`. The limit in the same breath: lifting it fixes *egress* only, and the receive-side residue in the header had an accepted membership on the interface the frames arrived on and still got none of them, so this ceiling closing would not by itself make that segment work.
    ...
    const beacon = new Beacon({
      socket,
      port,
      nonce: hypercoreCrypto.randomBytes(8),
      group,
      socketPort,
      memberships,
      onpeer: opts?.onpeer
  ...
  _onmessage (msg, rinfo) {
    ...
    //
    // Still discarded as a peer, and now read on the way out for the one fact a
    // lone host can measure about its own multicast: the source address the
    // kernel stamped on it is the interface the frame left on. First echo wins —
    // see the header for why that is the replay-resistant choice as well as the
    // simpler one — and no state is derived from it, so a wrong answer here
    // misinforms an operator and changes nothing else.
    if (beacon.nonce === this.nonce.toString('hex')) {
      if (this.announcing === null && rinfo.family === 'IPv4' && typeof rinfo.address === 'string') {
        this.announcing = rinfo.address
      }
      return
    }
...
class Lan {
   ...
   *
   * **That is also why a firewalled device is indistinguishable from an empty
   * segment, and it is the first thing to suspect.** Measured host to host: a
   * Windows 11 machine drops this beacon inbound under its default firewall, with
   * the datagrams reaching the NIC and dying at WFP — so the sender sees a clean
   * `send`, the receiver's socket is bound and joined, and both ends look healthy
   * while nothing is delivered. `OPERATIONS.md` §9 carries the measurement and the
   * rule an operator has to add. Deliberately *not* turned into an error here: this
   * function cannot tell "nobody is listening" from "somebody is listening and
   * cannot hear me", and a warning that fires on every genuinely-alone device is a
   * warning an operator learns to ignore before the day it is true.
   *
   * **There is now a second cause with the same silence, and it is on this side of
   * the wire.** Measured across a direct segment: the frames arrived on the
   * interface, the interface held an accepted membership, and the socket delivered
   * none of them — and separately, this host announces on `en0` while the peer is
   * on `en8`. So the three suspects behind an empty answer are the neighbour's
   * firewall, this device announcing on the wrong interface, and this device not
   * being handed what did arrive. `beacon.announcing` distinguishes the second
   * from the other two without a second machine; nothing here distinguishes the
   * first from the third, which is why this still resolves rather than warns.
   *
