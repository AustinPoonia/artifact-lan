 ...
 * us at; `nodes` are added to the routing table and never sampled. A device that
 * can reach both the public DHT and the segment gets *different* answers from
 * each — its public address and its RFC1918 one — and enough disagreement makes
 * the sampler give up and report no address at all. So LAN nodes join the
 * bootstrap list only when it would otherwise be empty, and are otherwise pure
 * routing. Auto-detecting a blocked DHT instead was considered and dropped: a
 * probe cannot tell blocked from slow, and guessing wrong degrades the path that
 * works today.
 *
 * @param {Lan | null} lan
 * @param {any[] | undefined} bootstrap
